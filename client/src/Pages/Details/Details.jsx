...
export const Details = () => {
  ...
  const editComment = (commentId, commentText) => {
    movieReview(commentId); // set the selected comment ID when the user clicks the "Edit" button
    setInputValue(commentText); // set the input field value to the current comment text
  };
  
  const updateComment = async (commentId, newCommentText) => {
    try {
                ...
                {allDetails.reviews.map((e) => {
                  return (
                    <div className="">
                      ...
                      <h1 className="mx-2">{e.comment}
                      <div>
                        {/* {e.map((comment) => (
                          <div key={comment.id}>
                            <p>{comment.text}</p>
                            <button onClick={() => editComment(comment.id, comment.text)}>Edit</button> // add an "Edit" button
                          </div>
                        ))} */}
                      </div>
                      </h1>
